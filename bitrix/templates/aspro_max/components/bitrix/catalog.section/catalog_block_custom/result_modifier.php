...
//для отобрежения офферов в виде селекта
...
if(is_array($arItem["OFFERS"]) && !empty($arItem["OFFERS"]))
{
	...
	$arOffersIblock = CIBlockPriceTools::GetOffersIBlock($arItem["IBLOCK_ID"]);
	...
	while($arOfferProperties = $dbOfferProperties->Fetch())
	{
		if (!in_array($arOfferProperties["CODE"],$arParams["OFFERS_PROPERTY_CODE"]))
			continue;
			$arIblockOfferProps[$arItem['ID']][] = array("CODE" => $arOfferProperties["CODE"], "NAME" => $arOfferProperties["NAME"]);
			$offerPropsExists = true;
	}

	/////

	// foreach($arIblockOfferProps as $key => $arArrCode)
	// {			
		foreach($arIblockOfferProps[$arItem['ID']] as $keycode=>$arCode){
		// foreach($arArrCode as $keycode=>$arCode){
			$emptyProp = true;
			...
			if ($emptyProp)
				unset($arIblockOfferProps[$arItem['ID']][$keycode]);				
				// unset($arArrCode[$keycode]);				
		}	
		// $arIblockOfferProps[$key] = $arArrCode;	
	// }
	
	/////
	
	foreach($arItem["OFFERS"] as $idOffer=>$arOffer)
	{
		
		$arSkuTmp = array();
		if ($offerPropsExists)
		{
			foreach($arIblockOfferProps[$arItem['ID']] as $key => $arCode)
			{
				
				if (!array_key_exists($arCode["CODE"], $arOffer["PROPERTIES"]))
				{
					$arSkuTmp[$arItem["ID"]][] = GetMessage("EMPTY_VALUE_SKU");
					continue;
				}
				...
				elseif (is_array($arOffer["PROPERTIES"][$arCode["CODE"]]["VALUE"])){
					$arSkuTmp[$arItem["ID"]][] = implode("/", $arOffer["PROPERTIES"][$arCode["CODE"]]["VALUE"]);
					
				}
				else{
					$arSkuTmp[$arItem["ID"]][] = $arOffer["PROPERTIES"][$arCode["CODE"]]["VALUE"];
		...
		else
		{
			if (in_array("NAME", $arParams["OFFERS_FIELD_CODE"]))
				$arSkuTmp[$arItem["ID"]][] = $arOffer["NAME"];
				else
					break;
		}
		$arSkuTmp[$arItem["ID"]]["ITEM_ID"] = $arItem["ID"];
		$arSkuTmp[$arItem["ID"]]["ID"] = $arOffer["ID"];
		$arSkuTmp[$arItem["ID"]]["NAME"] = $arOffer["NAME"];
		foreach ($arOffer["PRICES"] as $code=>$arPrice)
		{
			if($arPrice["CAN_ACCESS"])
			{
				$arSkuTmp[$arItem["ID"]]["PRICES"][$code]["TITLE"] = ($pricesCount > 1) ? $arItem["CAT_PRICES"][$code]["TITLE"] : "";
				if ($arPrice["DISCOUNT_VALUE"] < $arPrice["VALUE"])
				{
					$arSkuTmp[$arItem["ID"]]["PRICES"][$code]["PRICE"] = $arPrice["PRINT_VALUE"];
					$arSkuTmp[$arItem["ID"]]["PRICES"][$code]["DISCOUNT_PRICE"] = $arPrice["PRINT_DISCOUNT_VALUE"];
				}
				else
				{
					$arSkuTmp[$arItem["ID"]]["PRICES"][$code]["PRICE"] = $arPrice["PRINT_VALUE"];
					$arSkuTmp[$arItem["ID"]]["PRICES"][$code]["DISCOUNT_PRICE"] = "";
				}
			}
		}
		...
		$arSkuTmp[$arItem["ID"]]["CAN_BUY"] = $arOffer["CAN_BUY"];
		if ($arOffer["CAN_BUY"])
			$allSkuNotAvailable = false;
			$arSkuTmp[$arItem["ID"]]["ADD_URL"] = htmlspecialcharsback($arOffer["ADD_URL"]);
			$arSkuTmp[$arItem["ID"]]["SUBSCRIBE_URL"] = htmlspecialcharsback($arOffer["SUBSCRIBE_URL"]);
			$arSkuTmp[$arItem["ID"]]["COMPARE"] = "";
			if (isset($_SESSION[$arParams["COMPARE_NAME"]][$arParams["IBLOCK_ID"]]["ITEMS"][$arOffer["ID"]]))
				$arSkuTmp[$arItem["ID"]]["COMPARE"] = "inCompare";
				$arSkuTmp[$arItem["ID"]]["COMPARE_URL"] = htmlspecialcharsback($arOffer["COMPARE_URL"]);
				if($arSkuTmp[$arItem["ID"]]["CAN_BUY"]) $arSku[$arItem["ID"]][$arOffer['ID']] = $arSkuTmp[$arItem["ID"]];
	}
	...
	if (CModule::IncludeModule('sale')) {
		...
		while($arBasket = $dbBasketItems->Fetch()) {
			...
			if(empty($arSku[$arItem["ID"]][$productId])) {
				continue;
			}
			if($arBasket["DELAY"] == "Y") {
				$arSku[$arItem["ID"]][$productId]["CART"] = "delay";
			}
			elseif ($arBasket["SUBSCRIBE"] == "Y" &&  $arNotify[SITE_ID]['use'] == 'Y') {
				$arSku[$arItem["ID"]][$productId]["CART"] = "inSubscribe";
			}
			else {
				$arSku[$arItem["ID"]][$productId]["CART"] = "inCart";
			}
		}
		$arSku[$arItem["ID"]] = array_values($arSku[$arItem["ID"]]);
	}
}

}//if ((!is_array($arIblockOfferProps) || empty($arIblockOfferProps)) && is_array($arSku) && !empty($arSku))
//print_r($arIblockOfferProps);
