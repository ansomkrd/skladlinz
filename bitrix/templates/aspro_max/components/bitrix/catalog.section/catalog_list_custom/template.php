						...
						<div class="information <?=($arItem["OFFERS"] && $arItem['OFFERS_PROP'] ? 'has_offer_prop' : '');?>  inner_content js_offers__<?=$arItem['ID'];?>_<?=$arParams["FILTER_HIT_PROP"]?>">
							...
							<?if($arItem["OFFERS"]){?>
								<?if(!empty($arItem['OFFERS_PROP'])){?>
									...
									<form name="sku_<?=$arItem['ID']?>" class="buyOffers">
										<div class="options" id="sku_selector_<?=$arItem['ID']?>">
									...
									<script>
										var arProperties<?=$arItem['ID']?>;
										var arSKU<?=$arItem['ID']?>;					              							               
										arProperties<?=$arItem['ID']?> =  [<?
											
											foreach($arResult["SKU_PROPERTIES"][$arItem['ID']] as $ar){
															?>{'CODE': '<?=$ar['CODE']?>', 'NAME':'<?=$ar['NAME']?>'},<?
														}?>],								            
											arSKU<?=$arItem['ID']?> = <?=CUtil::PhpToJsObject($arResult["SKU_ELEMENTS"][$arItem['ID']])?>;
										
											$(document).ready(function(){      
												buildSelect('sku_<?=$arItem['ID']?>', 'sku_selector_<?=$arItem['ID']?>', 0, arSKU<?=$arItem['ID']?>, arProperties<?=$arItem['ID']?>, "list", "cart"); 
							...
							<?elseif($arItem["OFFERS"]):?>
								...
								<?}else{?>
									<div class="offer_buy_block" data-tovar="<?=$arItem["ID"];?>">
										<div class="counter_wrapp list clearfix ce_cmp_hidden">
											...
											<div class="button_block <?=($arAddToBasketData["ACTION"] == "ORDER" || !$arAddToBasketData["CAN_BUY"] || !$arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_LIST"] || $arAddToBasketData["ACTION"] == "SUBSCRIBE" ? "wide" : "");?>">
												<!--noindex-->
													<span data-value="<?=$arItem["MIN_PRICE"]['DISCOUNT_VALUE'];?>" data-currency="RUB" class="to-cart btn btn-default transition_bg animate-load has-ripple" data-props="" data-item="<?=$arItem["ID"];?>" data-float_ratio="" data-ratio="1" data-bakset_div="bx_basket_div_<?=$arItem["ID"];?>" data-part_props="Y" data-add_props="" data-empty_props="" data-offers="N" data-iblockid="55" data-quantity="1">
													...
													<a rel="nofollow" href="/basket/" class="in-cart btn btn-default transition_bg" data-item="<?=$arItem["ID"];?>" style="display:none;">
													...
													<span class="hidden" data-js-item-name=""></span>
													<!--/noindex-->
											</div>
										</div>
