								...
								<?$frame = $this->createFrame()->begin('');?>
									<div class="">
										...
										<?if($arResult["OFFERS"] && $showCustomOffer):?>
											<div class="buy_block offer-props-wrapper" data-tovar="<?=$arResult["ID"];?>">
												...
												<div class="sku_props">
													<form name="sku" class="buyOffers">
															
															<div class="options" id="sku_selector_0">
																<input type="hidden" value="" name="arIdOffer[]" />
																<div class="select row"></div>
																<div style="clear:both"></div>
															</div>
													</form>
													<span class="addOffer icon-text grey s35" data-number="0">
														<i class="fa fa-plus"></i>
														Добавить еще упаковку
														</span>
												</div>
											</div>
										<?endif;?>
												...
												<div class="offer_buy_block buys_wrapp">
													<div class="counter_wrapp big list clearfix">
														...
														<div id="<?//=$arItemIDs["ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>" class="button_block <?=($arAddToBasketData["ACTION"] == "ORDER" || !$arAddToBasketData["CAN_BUY"] || !$arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_DETAIL"] || $arAddToBasketData["ACTION"] == "SUBSCRIBE" ? "wide" : "");?>">
															<!--noindex-->
																<?//=$arAddToBasketData["HTML"]?>
																<span data-value="<?=$arResult['MIN_PRICE']['VALUE']?>" data-currency="RUB" class="btn-lg to-cart_custom btn btn-default transition_bg animate-load has-ripple" 
																data-props="" data-item="<?=$arResult["ID"];?>" data-float_ratio="" data-ratio="1" data-bakset_div="bx_basket_div_<?=$arResult["ID"];?>" data-part_props="Y" data-add_props="Y" data-empty_props="" data-offers="N" data-iblockid="55" data-quantity="1"><i></i><span>В корзину</span></span>
																<a rel="nofollow" href="/basket/" class="btn-lg in-cart btn btn-default transition_bg has-ripple" data-props="" data-item="<?=$arResult["ID"];?>" style="display:none;"><i></i><span>В корзине</span></a>
															<!--/noindex-->
														</div>
													</div>
													<?if(isset($arCurrentSKU['PRICE_MATRIX']) && $arCurrentSKU['PRICE_MATRIX']) // USE_PRICE_COUNT
...
<script>
	 <?  /* var rose =  [<?
        $ccc=8;
        foreach($arResult["SKU_PROPERTIES"] as $ar){
            ?>{'IDD': '<?=$ccc?>'},<?
        $ccc++;
        }?>
    {'IDD': '<?=$ccc++?>'},];
    ...
    var ava =  [    
        <?foreach($arResult["OFFERS"] as $arOffer):?>
            {'ID':'<?=$arOffer['ID']?>', 'AVA': '<?=$arOffer['CATALOG_QUANTITY']?>'},
        <?endforeach;?>
    ,];*/?>
    ...
    $(document).ready(function(){        
            ...
            buildSelect('sku', 'sku_selector_0', 0, arSKU, arProperties, "detail", "cart"); 
			
			

			$('span.addOffer').on('click', function(e){
				var id = $(this).attr('data-number');
				var idNew = parseInt(id)+1;

				$(this).attr('data-number', idNew);

				var introHtml = '<input type="hidden" value="" name="arIdOffer[]" /><div class="select row"></div>';
				var del = '<div class="del" onclick="$(\'#sku_selector_'+idNew+'\').remove();"><i class="fa big-icon white s35 fa-times"></i></div><div style="clear:both"></div>';
				$("form[name=sku]").append('<div class="options" id="sku_selector_'+idNew+'"></div>');
				$('div#sku_selector_'+idNew).append(introHtml+del);
				
				buildSelect('sku', 'sku_selector_'+idNew+'', 0, arSKU, arProperties, "detail", "cart");
				//$('#sku_selectors_'+idNew+' select').not('.destroyed').trigger("refresh").styler();
